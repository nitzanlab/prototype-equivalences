...
root = '/cs/labs/yweiss/roy.friedmam/collabs/time-warp/'
...
def DFORM_classify(x, xdot, its, lr, n_layers, n_freqs, freeze_frac, path, true_sign, prototypes,
                   det_reg: float=1e-3, cen_reg: float=1e-3, weight_reg: float=1e-3, noise: float=0):
    ...
    subplot = 1
    for (a, omega) in prototypes:
        # ============================================ fit prototype ==========================================
        prototype = get_oscillator(a=a, omega=omega)
        H = Diffeo(dim=2, n_layers=n_layers, K=n_freqs, actnorm=False).to(device)
        H, loss, ldet, score = fit_prototype(H, x.clone().to(device), xdot.clone().to(device), prototype, its=its,
                                             lr=lr, verbose=False, freeze_frac=freeze_frac, weight_decay=weight_reg,
                                             det_reg=det_reg, center_reg=cen_reg, noise=noise)
...
@click.option('--det_reg',     help='regularization on determinant', type=float, default=0)
@click.option('--cen_reg',     help='regularization on center', type=float, default=0)
@click.option('--weight_reg',  help='weight decay to use', type=float, default=1e-4)
@click.option('--omega',       help='angular velocity to use', type=float, default=.5)
def classify_all(type: str, n: int, job: int, lr: float, its: int, n_layers: int,
                 n_freqs: int, fr_rat: float, noise: float, alg_noise: float, rep: int,
                 det_reg: float, cen_reg: float, weight_reg: float, omega: float):
    ...
    # ============================= define paths ===========================================#
    path = root + (f'results/SPE/{type}_TWAgrid/its={its}_lr={lr:.0E}_layers={n_layers}_freqs={n_freqs}_'
                   f'noise={noise:.2f}_algnoise={alg_noise:.2f}_'
                   f'det={det_reg:.0E}_cenreg={cen_reg:.0E}_wreg={weight_reg:.0E}_fr={fr_rat:.2f}'
                   f'/rep={rep}/')
    ...
    data = np.load(root + f'data/{type}/X_test.npy')
    true_params = np.load(root + f'data/{type}/sysp_test.npy')
    true_labels = np.load(root + f'data/{type}/topo_test.npy')

    data = data[job*n:job*n+n]
    true_params = true_params[job*n:job*n+n]
    true_labels = true_labels[job*n:job*n+n]

    res_dict = {'prototypes': prototypes,
        'params': [],
        'true_labels': [],
        'losses': [],
        'logdets': [],
    ...
    for i in tqdm(range(n)):
        ...
        results = DFORM_classify(x, xdot, its=its, lr=lr, n_layers=n_layers, n_freqs=n_freqs,
                                 ...
                                 true_sign=-1 if true_labels[i][0]==4 else 1, prototypes=prototypes,
                                 noise=alg_noise, weight_reg=weight_reg, det_reg=det_reg, cen_reg=cen_reg)
